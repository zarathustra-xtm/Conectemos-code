...
<script language="Javascript">
    window.location = "?enc=_subir-cont&con=<?php echo "".htmlspecialchars($_GET['con'])."";  ?>";
</script>    
        <?php 
            }
?>	
...
?>		
<?php
$file = "_cont/cont_".htmlspecialchars($_GET['con'])."/";
if(is_dir($file))
  {
  echo ("");
  }
  ...
  {
  mkdir("_cont/cont_".htmlspecialchars($_GET['con'])."/", 0777);
  }

?>
<?php 
$count = 0;
$dir = opendir( "_cont/cont_".htmlspecialchars($_GET['con'])."/");
...
while (false !== ($fname = readdir( $dir ))) {
	for($i=0; $i < count($_FILES['files']['name']); $i++) {
	  ...
	  if ($tmpFilePath != ""){
		...
		$newFilePath = "_cont/cont_".htmlspecialchars($_GET['con'])."/" .$fecha."-".$_FILES['files']['name'][$i];
		$resizeWidth = 650;
		$thumbWidth = 650;

		if(move_uploaded_file($tmpFilePath, "_cont/cont_".htmlspecialchars($_GET['con'])."/".$realname)) {
			...
			imagejpeg( $tmp_img, "_cont/cont_".htmlspecialchars($_GET['con'])."/".$realname, 100 );
...
<div class="space-20"></div>                      
    <label class="pull-right inline">
        <a href="?enc=_cont" class="btn btn-danger btn-sm">
    ...
    <div class="widget-body">
        <div class="widget-main padding-8">
            <div id="profile-feed-1" class="profile-feed">
                <?php
                $ruta = "_cont/cont_".htmlspecialchars($_GET['con'])."/"; // Indicar ruta
                echo " $ruta  ";
                            ...
                            $maso = $file ;
                                    ...
                                    <div class="profile-activity clearfix">
                                        ...
                                        <div class="tools action-buttons">
                                                ...
                                                <a href="?enc=_subir-cont&del=_1&con=<?php echo "".htmlspecialchars($_GET['con'])."" ;?>&nom=<?php echo $file ;?>" class="red"><i class="ace-icon fa fa-times bigger-125"></i>
                                                </a>
                                        </div>
                                    </div>                                
                                           <?php
...
<div class="hr hr2 hr-double"></div>  
        ...
        switch($page)
			{
				case '_1':
					unlink("_cont/cont_".htmlspecialchars($_GET['con'])."/".htmlspecialchars($_GET['nom'])."");
                ?>
<script language="Javascript">
    window.location = "?enc=_subir-cont&con=<?php echo "".htmlspecialchars($_GET['con'])."";  ?>";
</script>
