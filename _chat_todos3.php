...
<?php 
$sql = "SELECT chatid,dona_id,sender_userid,message,reciever_userid,timestamp from ".$tutu."_chat2 where sender_userid like '".htmlspecialchars($_GET['env'])."' or reciever_userid like '".htmlspecialchars($_GET['env'])."' and sender_userid like '".htmlspecialchars($_GET['rec'])."' and dona_id like '".htmlspecialchars($_GET['khat'])."' order by chatid asc limit 30";
