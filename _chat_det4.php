...
if( isset($_GET['activo'])  )
     {
        ...
        echo "Record updated successfully";
            ...
            header('Location:?enc=_det4&a='.htmlspecialchars($_GET['a']).'&s='.htmlspecialchars($_GET['s']).'');
        }
?> 	
...
$(document).ready(function() {
    ...
    $('#chat').load('_chat_todos4.php?env=<?php echo $orgo;?>&rec=<?php echo $reo;?>&khat=<?php echo $ayu;?>');//actualizas el div
   }, 2000 );
});
...
<br>            
        ...
        <section class="sample-text-area">
            ...
            <div class="area-heading row">
                <div class="col-md-12">
                <?php
                    $sql="SELECT id_org,n_org FROM ".$tutu."_org where id_org like '".htmlspecialchars($_GET['s'])."'";
                    $result = $db->query($sql);

                    if ($result->num_rows > 0) {
                        // output data of each row
                        while($valores2 = $result->fetch_assoc()) {
                            $runu = $valores2['n_org'] ;
                        }
                    } else {
                        echo "0 results";
                ...
                <?php                    
                    if ($stdona == '1'){
                ?>                     
                    <a href="?enc=<?php echo htmlspecialchars($_GET['enc']);?>&a=<?php echo htmlspecialchars($_GET['a']);?>&s=<?php echo htmlspecialchars($_GET['s']);?>&don=donado" class="btn btn-success btn-block dropdown-toggle tooltipss"  role="button" aria-haspopup="true" aria-expanded="false">Realicar donaci&oacute;n a: <?php echo $runu ;?>
                        <div class="bottom">
                            <p>Al hacer clic aqu&iacute;, usted asignar&aacute; el recurso<br> que esta dispuesto a donar con el usuario: <?php echo $runu ;?> ,<br>esto lo compromete a ponerse de acuerdo con <br>el usuario para la entrega inmediata<br> o parcial del recurso, por favor<br> sea consciente y cumpla con sus donaciones, Muchas Gracias.</p>
                        </div>	
                    </a> 
                <?php
                    } elseif ($stdona == '99'){ 
                ?>                     
                    <a href="?enc=<?php echo htmlspecialchars($_GET['enc']);?>&a=<?php echo htmlspecialchars($_GET['a']);?>&s=<?php echo htmlspecialchars($_GET['s']);?>&activo=<?php echo htmlspecialchars($_GET['a']);?>" class="btn btn-info btn-block dropdown-toggle tooltipss"  role="button" aria-haspopup="true" aria-expanded="false">Donaci&oacute;n asignada a: <?php echo $runu ;?>
                        <div class="bottom">
                            <p>Haga clic para quitar esta asignaci&oacute;n.</p>
                        </div>	
                    </a>                     
                <?php
 ...
 <?php
    if( isset($_GET['don']) )
         { 
            $sql = "UPDATE ".$tutu."_dona SET asig_dona = '".htmlspecialchars($_GET['s'])."', fedona_dona = '".$feca."',st_dona = '99' WHERE id_dona like '".htmlspecialchars($_GET['a'])."' ";
